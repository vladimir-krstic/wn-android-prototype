...
| 4f | Restorable external camera and key export | Complete; camera capture retains its output destination across Activity recreation and reports launch failure inline; raw/encrypted key export retains a request descriptor rather than transient or saveable secret payload, derives content only after destination return, and preserves the existing write-error UI; static gate passes | Pending commit |
