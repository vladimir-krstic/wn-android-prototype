...
| C046 · Outbound share and save from message actions | Implemented; host verified within the prototype boundary | Choose, cancel or retry; Back dismisses the inner surface without sending | Message actions share text, White Noise profile identifiers and actual local attachment bytes, with per-frame save results, retry and cancelled/unavailable/no-handler outcomes. Save uses explicit document destinations; mixed sharing requires confirmation and reports only Android handoff, never delivery. Filenames/MIME types are preserved or truthfully re-encoded. Saved items are not replayed; queued document state survives recreation. |
