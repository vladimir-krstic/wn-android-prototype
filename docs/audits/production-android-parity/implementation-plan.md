...
- B12: implemented and host-verified 2026-09-04; 399 unit tests, zero lint errors, both APKs and nine new compiled UI/platform cases. [Selected brief and evidence](../../screens/draft-photo-editor.md). Commit title: `B12: Add reversible draft photo editing`.
...
| **B12** | Draft photo editor | C050 | B11 | Implemented; host verified |
...
Completed and host-verified 2026-09-04; [implementation evidence](../../screens/conversation-history-and-reading.md#implementation-evidence). Device/visual acceptance remains pending.
...
**Status:** Implemented and host-verified; [evidence](../../screens/text-attachments-and-shared-content.md#implementation-evidence). Strict bounded reading, Copy/selection/speech, newest-first media filters/month groups, source-owned voice playback and local package outcomes are implemented. 414 host unit tests pass; 13 new UI/platform cases compile. Q06 still excludes real installation; device/visual acceptance remains pending.
...
**Status:** Implemented and host-verified; [evidence](../../screens/location-sharing.md#implementation-evidence). Explicit coordinate/review and current-location outcomes, draft/reply-safe send/retry, source-compatible offline cards and Maps handoff/error recovery are implemented. 432 host unit tests pass; 13 new UI/platform cases compile. Real GPS, tiles/geocoding and device/visual acceptance remain outside Q06 coverage.
...
**Status:** Implemented and host-verified; [evidence](../../screens/read-aloud-transport.md#implementation-evidence). One foreground queue, paused transport/seek, bounded history and retry, source-safe return, native range/chunk highlighting and accessible following are implemented. 465 host unit tests pass; 15 new UI/platform cases compile. Profile/background boundaries clear playback and return ownership. Device speech and visual acceptance remain pending.
...
**Implementation:** Prototype configures device-authentication preference but lacks lock surface/session gating. Add deterministic locked/authenticating/cancel/failure/unlocked states, background timing and no-data flash. Add learning-request preference and coverage for relevant app-owned inputs; wording must say request, not guarantee about keyboard behavior. Keep the established Recents-only promise and add a separate chat screenshot/recording control. Recents protection applies only while paused; chat capture protection applies to conversation and chat-detail routes; app lock remains independently secure. Existing diagnostic exports are sanitized. Production forensic files may contain message/identity/device data. Keep consent, sensitive export and sanitized diagnostics separate; do not reuse reassurance copy.
...
**Status:** Implemented and host-verified; [selected brief](../../screens/relay-publication-and-validation.md#implementation-evidence). Existing capabilities preserved; no device or visual verification.
