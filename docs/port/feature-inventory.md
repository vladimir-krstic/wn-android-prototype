...
| Speech messages | Hold to record, review, deterministic waveform/sample, Transcribe, Voice/Text/Both formats, playback | Verified 2026-09-01 — the complete eight-entry pinned voice timeline is restored, including 7 s, 18 s, 1:22, and Both/transcript cases. The deterministic local recording/review flow, waveform, hold threshold, playback, remaining-duration formatting, transcription, editing, format selection, and single-result submission pass the physical Pixel 8a suite; user visual acceptance remains separate |
