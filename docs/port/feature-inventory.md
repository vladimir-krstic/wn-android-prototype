...
references resolve inside message text with member-aware presentation and
profile routing. Public `note`, `nevent`, and `naddr` references render typed
note, article, image, video, document, and event cards with bounded loading,
invalid, unavailable, not-found, failure, and exact-revision retry states.
Article and document readers preserve structured selectable source text; video
uses the existing foreground-owned Media3 surface. 826 unit tests pass; seven
UI cases compile, zero lint errors and both APKs build.
[Evidence](../screens/verified-event-cards-and-readers.md#implementation-evidence).
The fixtures add no relay, network, persistence, cryptography, or event-signature
claims; device and visual acceptance remain pending.

## 2026-09-04 production B29

C112/C113 are implemented and host-verified. Settings exposes four
public-key-only agent connector guides, manual setup and recoverable external
documentation. Conversations distinguish partial streaming text and ordinary
queued/running/completed/failed/cancelled/unavailable operation cards from raw
developer stream records. Exact-profile example injection is developer-only.
817 unit tests pass; seven UI cases compile, zero lint errors and both APKs
build. [Evidence](../screens/ai-agents-and-streaming-operations.md#implementation-evidence).
No connector, agent account, transport, Marmot, persistence or permission was
added; device and visual acceptance remain pending.
