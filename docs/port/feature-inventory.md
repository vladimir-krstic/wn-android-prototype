...
| Privacy & Security | Device authentication, Recents snapshot privacy, Erase App Data entry and availability | Reworked 2026-08-29 — separated Material device-protection group, Android Recents wording, real device-lock recovery, and Auto-lock revealed only while authentication is effective; Diagnostics and device-wide erase use concise rows with consequences outside the actions; static gate passed and device acceptance remains pending |
| Diagnostics & Improvements | Typed consumer privacy destination, independent choices, summary, stored size, confirmed clear | Reworked 2026-08-29 — concise summarized Privacy route; separated native switches without a redundant heading; Stored Diagnostic Logs appears only after records exist; platform-formatted size and confirmed clearing preserve preferences; large dark RTL regression compiles and static gate passed |
