...
- **Reference ready** — accepted source evidence is available at the pinned upstream reference.
...
The capability tables below are the original iOS parity ledger. Production
Android B01–B32 coverage and dated batch verification live in the
[capability matrix](../audits/production-android-parity/capability-matrix.md) and
[implementation record](../audits/production-android-parity/implementation-plan.md).
Historical test totals do not certify later builds; the latest engineering
results are in [the hardening audit](../codebase-hardening-audit.md).

The following approved 2026-09-05 refinements are implemented with host
verification. Associated UI tests were compiled; current device/visual acceptance
remains pending. The linked briefs own exact behavior, copy, and dated evidence.

| Area | Implemented refinements | Canonical brief |
| --- | --- | --- |
| Onboarding | Grouped key-entry actions, task progress cleanup, canonical Amber identity, no normal-startup loading flash | [Onboarding](../screens/onboarding-and-profiles.md), [Keys](../screens/keys-and-profile-exit.md) |
| Chats | Horizontal filters/folders, compact selection, main-avatar profile switching, native New Message recovery | [Chats and creation](../screens/chats-and-chat-creation.md), [Organization](../screens/chat-organization-and-recovery.md) |
| Folders | Live example rules, compact folder-management icon, shared picker alignment and selection rows | [Folders](../screens/chat-folders.md) |
| People | Conditional shared-groups entry, bottom invite action, eligible-group identity, profile action icons and nickname-field contrast | [People](../screens/people-discovery-and-private-details.md) |
| Global Search | Provider-owned voice recognition, microphone/clear controls, filter icon/context menu, no update callout during search | [Search](../screens/global-search.md) |
| Conversation history | Measured tail-jump geometry, viewport threshold, native down action, retained unread divider and simplified search controls | [History and reading](../screens/conversation-history-and-reading.md) |
| Message actions | Reduced contextual menu and selection/forwarding presentation | [Interactions](../screens/message-interactions-and-search.md) |
| Chat/Group Info | Scrolled identity, five-member preview/full roster, lifecycle-action order, native icon/spacing and action groups | [Information](../screens/chat-and-group-information.md), [Administration](../screens/group-administration-and-transcript.md) |
| Shared UI | Segmented rows, stable state shapes, modal choice surfaces, directory picker sheets, informational callouts, dialog contrast | [Shared settings](../screens/shared-settings-polish.md) |
| Profile media | Avatar viewer overflow controls | [Profile media](../screens/profile-media-and-lightning.md) |
| Appearance | Apply-button size and bubble-reset menu | [Appearance](../screens/appearance-typography-and-input.md) |
| Data Usage | Concise photo/voice-quality choices | [Downloads and quality](../screens/downloads-and-media-quality.md) |
| Disappearing messages | Concise countdown explanation | [Timers](../screens/disappearing-timers-and-expiry.md) |
| Updates | Compact Chats entry, summary placement, temporary update emblem, current-state card hidden | [Updates](../screens/distribution-gated-app-updates.md) |
| AI agents | Setup presentation and current shared component use | [AI agents](../screens/ai-agents-and-streaming-operations.md) |

The authorized 2026-08-31 cross-app hardening/device result is retained in the
[historical engineering audit](../codebase-hardening-audit.md). It does not
constitute current-build device verification.
...
category scope, Android settings fallback and staged vibration/override/preview
states. Native custom date/time and canonical foreground mute expiry preserve the
restore mode across all mute entries. Q06 keeps real delivery/services/publication,
vibration and persistence outside coverage. 714 unit tests, zero lint errors and
both APKs pass; ten new UI/platform cases compile only.
See [notification-controls.md](../screens/notification-controls.md#implementation-evidence).
...
save and failed history refresh are distinct. Messages keep their own send/read
countdowns through later timer changes. Expiry removes canonical content and
reconciles previews, read state, replies, media/readers, forwarding, transcripts
and speech. The fixed foreground clock also handles inactive signed-in profiles.
655 unit tests, zero lint errors and both APKs pass; nine new UI cases compile only.
See [disappearing-timers-and-expiry.md](../screens/disappearing-timers-and-expiry.md#implementation-evidence).
...
cases compile. Encoded Nostr reference resolution and typed public-event cards
are subsequently host-verified in B30. Historical device evidence below does
not verify this build.
...
| Agent messages | Partial streaming text and typed operation status, progress and details | B29 host verified — ordinary queued/running/completed/failed/cancelled/unavailable operation cards remain visible without Developer Tools; raw stream records stay behind Streaming Debug. [Evidence](../screens/ai-agents-and-streaming-operations.md#implementation-evidence). Visual acceptance pending. |
| Public profile and event references | Encoded profile resolution, member-aware mentions, typed public-event cards, readers and bounded recovery | B30 host verified — checksum-valid `npub`/`nprofile` references preserve authored source spans while resolving known profiles; member mentions, known nonmembers and unavailable profiles remain distinct. Strict `note`/`nevent`/`naddr` validation drives six typed card kinds plus loading/not-found/invalid/unavailable/failure/retry states. Article/document readers, image/event details, and existing Media3 video playback remain local and source-preserving. [Evidence](../screens/verified-event-cards-and-readers.md#implementation-evidence). Device and visual acceptance pending. |
...
| Disappearing messages | Header/list/row indicators, staged presets/custom timers, pruning and saved send/read expiry | Implemented — B20 host verified: bounded custom input, admin/owned confirmation, accepted refresh retry and canonical expiry cleanup; device/visual acceptance pending |
...
| AI Agents | Hermes/OpenClaw/OpenCode/Codex setup prompts, manual public-key setup and documentation | B29 host verified — active-profile `npub` prompt review/copy and unavailable-key/docs recovery use the established Settings system; no installation or account connection is performed. [Evidence](../screens/ai-agents-and-streaming-operations.md#implementation-evidence). Visual acceptance pending. |
| Notifications | Global delivery, permission/provider recovery, app-wide background preference, previews, categories and per-chat mode/mute/vibration | Implemented — B22 host verified: owned pending/failure/retry, real Android settings handoff with fallback, custom mute deadlines/expiry and independent restore choice; Q06 excludes delivery/services/publication/vibration. Device/visual acceptance pending; see [evidence](../screens/notification-controls.md#implementation-evidence) |
...
## Historical shared UI verification

The August 26–30 Chats/privacy, Expressive-menu, header, composer, and scroll
fixes are incorporated into the capability tables and their current briefs:
[Chats](../screens/chats-and-chat-creation.md),
[diagnostics consent](../screens/diagnostics-and-improvements.md),
[menus](../screens/app-menus.md),
[conversation](../screens/shared-conversation-core.md), and
[composer](../screens/composer-media-and-speech.md).
Historical validation details remain with those contracts and in the
[engineering audit](../codebase-hardening-audit.md). No new device acceptance is
implied by this documentation consolidation.
...
## Dated refinement evidence

These entries retain the sequence of accepted changes on September 5. Later
entries and the linked current screen briefs supersede earlier intermediate
presentations. Repository cleanup and current host verification are recorded in
the [September 6 engineering audit](../codebase-hardening-audit.md#2026-09-06-repository-cleanup).
...
2026-09-08 user-approved Android extension: personal floating message stack.
`FloatingMessageController` retains ordered account-owned message references;
`FloatingMessagesHost` presents compact native paging and an expanded full-bubble
sheet over chat/list routes. Context action, drag/edge snap, pagination, source
navigation, remove/clear and source lifetime cleanup are implemented separately
from shared pins. Host state tests and compiled `FloatingMessageFlowTest` provide
behavior evidence; see `docs/screens/floating-messages.md`. Device execution and
visual acceptance remain pending; no upstream parity claim is made for this extension.

2026-09-08 floating-stack follow-up: expansion now stays within `FloatingMessageCard`
and uses `ReadOnlyMessageContent` without a nested bubble. Whole-card long-press
feedback, 24 dp side margins and fixed header pagination (multiple messages only)
replace the initial sheet and expand icon. Updated flow tests cover these changes;
see the floating-messages brief for final host evidence and visual acceptance status.

2026-09-08 floating gesture correction: stationary message-side pagination, tap
cycling in either state, long-press expand/collapse toggle and no card press
indication. Flow tests updated for side-rail geometry and expanded-content gestures;
see the floating-messages brief for host verification.
