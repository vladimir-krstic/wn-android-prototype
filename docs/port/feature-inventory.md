...
| Timeline and clustering | Chronology, day boundaries, author and five-minute clusters, date markers, events, notices | Pixel 8a conversation-surface inspection passed 2026-08-31 — keyed lazy timeline, sticky date sections, deterministic ordering and cluster edges remain; initial/new-message positioning now settles the chronological end fully above the measured composer, and group identity aligns its 30 dp avatar to the bubble rather than reaction/time metadata. Focused geometry tests pass; user visual acceptance remains separate |
| Message states | Incoming/outgoing, sent/delivered/read/failed, retry, deleted/tombstoned, replies and missing fallbacks | Refined 2026-08-31 — tail-free 16 dp bubbles use the established 12/8 dp content insets, 2/16 dp cluster rhythm and terminal metadata. Failed delivery now occupies the exact outgoing timestamp slot with the same 12 dp edge inset, 2 dp top gap, 3 dp icon gap, `labelSmall` typography, and 14 dp status footprint; only its warning/label use the semantic error color, while tapping the message and the named accessibility action retain retry. Scoped deletion, reply resolution, action availability and typed details remain. Composer and message replies preserve the natural-height 3 dp accent/author/two-line content with an 8 dp message inset/radius; renewed device inspection and user visual acceptance remain pending |
| Text and links | Plain and multiline text, detected links, link preview, copy/search/read-aloud behavior | Link-preview geometry static gate passed 2026-08-31 — local first-HTTPS metadata, named suppression, copy/search and received-text Android TextToSpeech remain; the tonal composer preview now shares the quote/image 8 dp inset and 16 dp radius inside the 24 dp composer; visual inspection pending |
| Images and video | One-to-seven media layout coverage, aspect clamping, viewer paging, capture and selection | Icon-action viewer follow-up static gate passed 2026-08-31 — each visible album tile preserves exact frame identity into one chronological chat-wide pager with sender/time/position, no sent-media thumbnails, 1×–4× bounded photo zoom, Back/pager arbitration, and exact-frame Share, Forward, Save and Go to Message; bottom Share/Forward now use edge-aligned transparent 48 dp icon targets rather than wide filled buttons; video handoff, Photo Picker/external camera selection and ordered draft shelf remain; device inspection pending |
...
| Reactions | Quick reactions, full emoji picker, counts, current-profile selection and replacement | Refined 2026-08-31 by explicit user direction — emoji-only pills share a 31×23 dp minimum, counted and `+N` pills grow horizontally, visible neighbors keep 3 dp gaps, and the rail overlaps the bubble bottom by 9 dp while retaining expanded 48 dp minimum interaction targets. Rail/time keep 12 dp edge insets; timestamps consistently use the incoming start/left or outgoing end/right edge in LTR, mirrored with the message in RTL, and start 2 dp below text/media bubbles whether reactions exist or not. Timestamp text, Sent fill, and Sending progress use the lower-emphasis `outline` neutral; Sent keeps a 14 dp status container with 10 dp check artwork. The bubble grows to 340 dp while the summary shows at most four real types plus one `+N`, reducing real types further only when width requires it. Transcript and focused preview share that summary, focused accessories clear the lowest visible metadata while preserving equal 8 dp gaps, and Message Details lists every type/person. Singleton-count omission, current-profile semantics, explicit removal, configuration, picker and swap/reset/apply remain; model and compiled UI regressions pass, renewed device execution is pending |
| Replies and mentions | Reply context, deleted/missing fallback, group-member mention filtering | Reply-geometry static gate passed 2026-08-31 — action-owned reply draft/focus, cancellation and fallbacks share the 12/3/10 dp content treatment; the composer quote now uses an 8 dp inset and 16 dp radius inside its 24 dp surface, while message quotes use an 8 dp inset/radius inside 16 dp bubbles; group suggestions still derive from matching active members; device inspection pending |
...
| Attachments | Android-native photo/video picker, camera, document picker, preview/remove/viewer behavior | Draft-inclusion placement static gate passed 2026-08-31 — photo/album/GIF/file/contact/link/reply removal shares one 48 dp target with a 20 dp circle, 12 dp X, equal 6 dp corner inset and inner-only state layer; 72 dp utility cards retain contact names and suffix-preserving filenames; draft review uses clean selected-only thumbnail rings, hides a single-item rail, and anchors its 48/22 dp inclusion control to the fitted image with the visible check exactly 4 dp inside the bottom-end edges. The earlier inspected picker/menu behavior and WN-ANDROID-0104 permission-safe camera repair remain; renewed hands-on capture/rich-state inspection and user acceptance are pending |
...
| Recipient speech actions | Read Aloud, Transcribe, Show/Hide Transcript, Copy Transcript, local-only provenance | Pixel 8a focused-action inspection passed 2026-08-31 — received text/voice commands now live conditionally in the source-preserving action overlay; ordinary bubbles retain only playback, active read-aloud progress, or a deliberately revealed view-local transcript. Transcribe/reveal/hide/copy provenance and Android TextToSpeech stop/shutdown behavior remain; model and focused UI tests pass |
| Message actions | Long-press context, Reply, Forward, Copy, Select, Info, Delete, permissions and full action availability | Pixel 8a focused-action inspection passed 2026-08-31 — deliberate hold/haptic lifts the real bubble between quick reactions and its policy-ordered command surface, aligned by direction and shifted inside safe bounds; exceptionally tall sources scale within a 320 dp real-preview budget. Conditional Retry/Copy/speech actions, named accessibility alternatives, typed details and scoped confirmation remain; the detached generic message-action sheet is superseded |
...
| Shared media | Media/files/links categories, chronological index, unified viewer and unavailable-page rules | Forward-sheet polish static gate passed 2026-08-31 — Shared Content and the conversation use one chronological frame-level projection with stable message/attachment/image keys, exact album-tile selection, deleted/unavailable filtering, complete viewer actions, the shared polished media-forward sheet, and typed Go to Message; counted disclosure rows, empty state and rich lists remain; device inspection pending |
