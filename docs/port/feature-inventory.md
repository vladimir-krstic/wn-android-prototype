...
| Message states | Incoming/outgoing, sent/delivered/read/failed, retry, deleted/tombstoned, replies and missing fallbacks | Reply-geometry static gate passed 2026-08-31 — semantic bubbles, terminal time/state, retry, scoped deletion, reply resolution, action availability and typed details remain; composer and message replies share the same natural-height 3 dp accent/author/two-line content while message quotes use an 8 dp inset/radius inside the 16 dp bubble; renewed visual acceptance pending |
...
| Selection and forwarding | Multi-select mode, target selection, ordered copies, media and keyboard behavior | Forward-sheet edge/high-state static follow-up passed 2026-08-31 — existing explicit selection/limits/order remain; all forwarding uses the shared 48 dp compact search, a 16 dp-inset segmented destination group with 2 dp separators, whole-row Checkbox semantics and trailing-check-only selection. The full rounded cap, Material handle area and fixed title/search region darken together after scroll while the lighter list canvas reaches the physical bottom edge. Media forwarding combines its optional four-line message and count-bearing action in a chat-matched 48 dp-minimum capsule with a 48/32 dp trailing arrow; the capsule independently clears navigation, measured/system-safe end padding brings the final row fully above it, and the modal can expand to a rounded 88%-height high state without becoming full screen. Exact-frame validation and single-Photo normalization remain; device inspection pending |
